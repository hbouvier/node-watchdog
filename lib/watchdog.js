module.exports = function () {
    ...
    Watchdog.prototype.save = function () {
        var $this = this;
        this.writeFile(this.filename, JSON.stringify(this.processes)).then(function () {
        }).fail(function (reason) {
            $hits.logger.log('error', 'save|filename=%s|data=%j|reason=%j', $this.filename, $this.processes, reason, $this.meta);
        }).done();
    };
    ...
    Watchdog.prototype.writeFile = function (filename, data) {
    ...
    Watchdog.prototype.modify = function (name, process) {
        var process = this.processes[name];
        if (process) {
            this.logger.log('modify|name=%s|terminating', name, this.meta);
            this.terminate(name);
        }
        this.process[name] = process;
        this.update(this.processes);
        this.save();
    };

    Watchdog.prototype.remove = function (name) {
        var process = this.processes[name];
        if (process) {
            this.logger.log('remove|name=%s', name, this.meta);
            this.terminate(name);
    ...
    function start(config, app) {
        ...
        return {
            ...
            save     : function () { return watchdog.save(); },
            add      : function (name, process) { return watchdog.modify(name, process); },
            remove   : function (name) { return watchdog.remove(name); },
            modify   : function (name, process) { return watchdog.modify(name, process); },
            start    : function (name) { 
